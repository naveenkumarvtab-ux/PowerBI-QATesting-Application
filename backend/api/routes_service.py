    ...
    try:
        ...
        if "naming" in checks:
            ...
            try:
                ...
                # Mobile Layout Compliance Check
                try:
                    from backend.core.mobile_layout_auditor import MobileLayoutAuditor
                    mob_res = MobileLayoutAuditor.audit_mobile_layout(layout_str, parsed_meta.get("pages", []))
                    ...
                    print(f"Service mobile audit error: {me}")

                # DAX Storage Engine vs Formula Engine Check
                try:
                    from backend.core.dax_performance_profiler import DaxPerformanceProfiler
                    dax_prof = DaxPerformanceProfiler.profile_all_measures(dax_measures, dax_columns)
                    ...
                    print(f"Service DAX profiler error: {de}")
