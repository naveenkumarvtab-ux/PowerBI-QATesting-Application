...
export default function ServiceTest() {
  ...
  // NOTE: redirect URI MUST be plain origin (no # hash, no trailing slash) - the hash fragment is never sent to Azure
  // Azure always strips the fragment. We forward internally via App.jsx after the redirect lands.
  const CLEAN_REDIRECT_URI = window.location.origin; // e.g. http://localhost:5173
  const [customRedirectUri, setCustomRedirectUri] = useState(CLEAN_REDIRECT_URI);
    ...
    const handleCallback = async () => {
      ...
      if (searchString) {
        ...
        if (code) {
          ...
          setError(null);
          try {
            // CRITICAL: must use the exact same redirect_uri as was used in the auth request
            // The hash (#) is never sent to Azure, so we always use plain origin
  ...
  const handleMicrosoftSignIn = async () => {
    ...
    try {
      // Always use plain origin as redirect_uri — hash fragments are never sent to Azure
      const redirectUri = window.location.origin;
